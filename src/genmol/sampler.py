...
def load_model_from_path(path):
    ...
    # if model.surf_encoder is not None:
    #     model.surf_encoder.surf_aa_features, model.surf_encoder.encoder, model.surf_encoder.pooling_encoder = model.surf_encoder.surf_aa_features.cuda(), model.surf_encoder.encoder.cuda(), model.surf_encoder.pooling_encoder.cuda()
    model.backbone.eval()
    if model.ema:
...
class Sampler:
    ...
    @torch.no_grad()
    def generate(self, x, softmax_temp=1.2, randomness=2, fix=True, gamma=0, w=2, condition=None):
        x = x.to(self.model.device)
        num_steps = max(self.mdlm.get_num_steps_confidence(x), 2)
        attention_mask = x != self.pad_index
        
        for i in range(num_steps):
            logits = self.model(x, attention_mask, condition=condition)
    ...
    @torch.no_grad()
    def conditional_generation(self, condition, num_samples=1, softmax_temp=0.8, randomness=0.5):
        # Prepare Fully Masked Inputs
        x = torch.hstack([torch.full((1, 1), self.model.bos_index),
                          torch.full((1, 1), self.model.eos_index)])
        x = self._insert_mask(x, num_samples, min_add_len=40)
        x = x.to(self.model.device)
        input_rep = condition.get('input_rep', None)
        if input_rep is not None:
            input_rep = input_rep.to('cuda')
            sample_length = torch.tensor([input_rep.shape[1]]).to('cuda')
            if self.model.rep_emb_pooling is not None:
                input_rep = self.model.rep_emb_pooling(input_rep, sample_length)
        surf_rep = condition.get('surfprop', None)
        if surf_rep is not None:
            surf_rep = surf_rep.to('cuda')
        input_prop = condition.get('input_prop', None)
        if input_prop is not None:
            input_prop = input_prop.to('cuda')
        if input_rep is not None and surf_rep is not None:
            cond_vec = self.model.rep_emb_encoder(input_rep) + self.model.surf_emb_encoder(surf_rep)
        elif input_rep is not None:
            cond_vec = self.model.rep_emb_encoder(input_rep)
        else:
            cond_vec = None
        if input_prop is not None:
            prop_vec = self.model.prop_emb_encoder(input_prop)
        else:
            prop_vec = None
        if cond_vec is not None and prop_vec is not None:
        ...
        cond_vec = cond_vec.repeat(num_samples, 1)
        condition = cond_vec.detach()
        return self.generate(x, softmax_temp, randomness, condition=condition)
